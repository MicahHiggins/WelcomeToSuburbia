...
func _spawn_player(id: int, player_scene: PackedScene) -> Node3D:
	...
	# If server and a level is already loaded, snap the new player to spawn
	var lfm: Node = get_node_or_null("../LevelFlowManager")
	if lfm != null and multiplayer.has_multiplayer_peer() and multiplayer.is_server():
		if lfm.has_method("server_place_player_if_needed"):
			lfm.call("server_place_player_if_needed", p3d)
